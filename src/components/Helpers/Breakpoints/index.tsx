...
type Screens = { [key in keyof typeof screens]?: number | string }
...
export const whichBreakpoint = (windowSize: number) => {
  ...
  if (width < parseInt(screens['2xl']) && width > parseInt(screens.xl)) return 'xl'
  if (width > parseInt(screens['2xl'])) return '2xl'
  return 'xs'
}
...
export const breakpoints = (values: Screens, windowSize: number) => {
  ...
  if (breakpoint in Object.keys(values)) {
